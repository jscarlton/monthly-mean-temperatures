...
Once you enter a station, starting, and ending year, the script will create a data directory (assuming it doesn't already exist) and create a file called historicalMonthlyMeans\_STATION\_STARTYEAR-ENDYEAR.csv. If the file already exists, it will append onto the existing file. Note that this process takes some time since it runs each query one after another. Patience remains a virtue.